...
class KevsDriver:
	...
	def process_training_post_entries(self, brand, post_entities_list):
		for post_entities in post_entities_list:
            ...
            # Ex for label creation
            #catagories list contains a list of list of images, each list of images is associated with a catagory
            catagatories_list = []
            images = []
            labels = []
            label_map = {}
            for label in range(len(catagories)):
                label_map[catagories] = label
                for catagory_images in catagatories_list:
                    # list comprehension below create a list of length
                    current_catagory_labels = [label] * len(catagory_images)
                    labels.extend(current_catagory_labels)
                    images.extend(catagory_images)
