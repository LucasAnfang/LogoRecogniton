...
class ConvertTF(object):
    ...
    def write_tfrecord(self, database_dir, images, labels, name):
    ...
    def convert_to_tfrecord(self, output_dir, image_paths, image_category_index):
        # prepare data for tfrecord
        print(image_paths)
        _NUM_VALIDATION = int(len(image_paths)*.2+1)
        _RANDOM_SEED = 0
        _NUM_SHARDS = 5 #?

        images = np.array(image_paths)
        labels = np.array(image_category_index)
